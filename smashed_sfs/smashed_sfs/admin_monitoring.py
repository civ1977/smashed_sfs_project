...
def online_users_view(request):
    ...
    def build_school_block(school_name, profile_id):
        ...
        section_ids = list(Section.objects.filter(school_profile_id=profile_id).values_list('section_id', flat=True)) if profile_id is not None else []
        profile_students = Student.objects.filter(section_id__in=section_ids).order_by('surname', 'name') if section_ids else []
        ...
        student_rows = []
        for s in profile_students:
            account = accounts_by_lrn.get(s.lrn)
            if account is None:
                student_rows.append({'name': f'{s.surname}, {s.name}', 'online': False, 'last_seen': None, 'label': 'No portal account'})
            else:
                student_rows.append({'name': f'{s.surname}, {s.name}', **_status(account.last_seen, now)})
